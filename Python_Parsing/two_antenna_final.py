...
directory = '/mnt/HDD1/Channel_Sensing_Raw_Data/Experiments_1_Classroom/location_1/Injector_1/Channel_1/20MHz/Synced/'
...
for file in files:
    ...
    CSI = np.load(file)
    CSI_base_one = CSI[0, :245]
    CSI_base_two = CSI[0, 245:]
...
for file in files_Antenna_Separated:
    ...
    for index, value in Antenna_Sync:
         if (str(index[-7:-4]) != str(file)[-9:-6]):
             A.append(value)
             # Delete the exact index from the CSI array
    CSI = np.delete(CSI, A, axis=0)
    print(np.shape(CSI))
    np.save(file, CSI)
